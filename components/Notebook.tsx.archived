...
const INITIAL_CELL: Cell = {
  id: 'init-cell',
  type: 'code',
  content: 'import numpy as np\nimport pandas as pd\nimport matplotlib.pyplot as plt\n\nprint("Welcome to Nebula Notebook!")\nprint("Try creating a dataframe to see the rich display:")\nprint("df = pd.DataFrame(np.random.randn(5, 3), columns=list(\'ABC\'))\\ndf")',
  outputs: [],
  isExecuting: false
};
...
export const Notebook: React.FC = () => {
  ...
  useEffect(() => {
    const initKernels = async () => {
      try {
        ...
        // Check if preferred kernel exists
        const kernelExists = kernels.some(k => k.name === preferredKernel);
        const kernelToStart = kernelExists ? preferredKernel : (kernels[0]?.name || 'python3');

        setCurrentKernel(kernelToStart);
        setKernelStatus('starting');

        await kernelService.startKernel(kernelToStart);
  ...
  // Initialize File System & Load Default
  useEffect(() => {
    const initFS = async () => {
      await initFileSystem([INITIAL_CELL]);
      ...
      if (activeId) {
        const content = await getFileContent(activeId);
        if (content) {
          loadFileAsync(activeId, content);
          return;
        }
      }
      ...
      if (fileList.length > 0) {
        const firstNb = fileList.find(f => f.fileType === 'notebook');
        if (firstNb) {
          const content = await getFileContent(firstNb.id);
          if (content) {
            loadFileAsync(firstNb.id, content);
          }
        }
      }
    };
  ...
  // Keyboard Shortcuts (Global Undo/Redo)
  useEffect(() => {
  ...
  const loadFileAsync = (id: string, content: Cell[]) => {
  ...
  const interruptKernel = async () => {
    try {
      await kernelService.interruptKernel();
      setExecutionQueue([]);
  ...
  // Execution Processor
  useEffect(() => {
    if (isProcessingQueue || executionQueue.length === 0 || !isKernelReady) return;
    ...
    const processNext = async () => {
      ...
      if (cell && cell.type === 'code') {
        ...
        try {
          await runPythonCode(cell.content, (output) => {
    ...
    processNext();
  }, [executionQueue, isProcessingQueue, isKernelReady, cells, setCells]);
