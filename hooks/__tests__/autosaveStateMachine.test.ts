/**
 * Tests for autosaveStateMachine - state machine for save operations
 */
...
describe('autosaveStateMachine', () => {
  ...
  describe('checking state transitions', () => {
    ...
    it('calculates delay based on content size for small files', () => {
      const state: AutosaveState = { status: 'checking' };
      const event: AutosaveEvent = { type: 'CHECK_COMPLETE', hasChanges: true, contentSize: 50 * 1024 }; // 50KB

      const { effects } = autosaveReducer(state, event);

      const scheduleEffect = effects.find((e) => e.type === 'SCHEDULE_SAVE') as { type: 'SCHEDULE_SAVE'; delay: number };
      expect(scheduleEffect).toBeDefined();
      expect(scheduleEffect.delay).toBeGreaterThanOrEqual(1000);
      expect(scheduleEffect.delay).toBeLessThanOrEqual(2000);
    });

    it('calculates delay based on content size for large files', () => {
      const state: AutosaveState = { status: 'checking' };
      const event: AutosaveEvent = { type: 'CHECK_COMPLETE', hasChanges: true, contentSize: 5 * 1024 * 1024 }; // 5MB

      const { effects } = autosaveReducer(state, event);

      const scheduleEffect = effects.find((e) => e.type === 'SCHEDULE_SAVE') as { type: 'SCHEDULE_SAVE'; delay: number };
      expect(scheduleEffect).toBeDefined();
      expect(scheduleEffect.delay).toBeGreaterThanOrEqual(5000);
    });
  ...
  describe('saving state transitions', () => {
    it('transitions to idle on SAVE_SUCCESS', () => {
      ...
      expect(newState.status).toBe('idle');
      expect(effects).toContainEqual({ type: 'CLEAR_BACKUP' });
  ...
  describe('getAutosaveDelay', () => {
    // Testing delay calculation through the state machine
    it('uses minimum delay for tiny files', () => {
      const state: AutosaveState = { status: 'checking' };
      const event: AutosaveEvent = { type: 'CHECK_COMPLETE', hasChanges: true, contentSize: 1024 }; // 1KB

      const { effects } = autosaveReducer(state, event);

      const scheduleEffect = effects.find((e) => e.type === 'SCHEDULE_SAVE') as { type: 'SCHEDULE_SAVE'; delay: number };
      expect(scheduleEffect.delay).toBeGreaterThanOrEqual(1000);
      expect(scheduleEffect.delay).toBeLessThanOrEqual(1500);
    });

    it('uses maximum delay for very large files', () => {
      const state: AutosaveState = { status: 'checking' };
      const event: AutosaveEvent = { type: 'CHECK_COMPLETE', hasChanges: true, contentSize: 100 * 1024 * 1024 }; // 100MB

      const { effects } = autosaveReducer(state, event);

      const scheduleEffect = effects.find((e) => e.type === 'SCHEDULE_SAVE') as { type: 'SCHEDULE_SAVE'; delay: number };
      expect(scheduleEffect.delay).toBe(60000); // Max delay
    });
  });
